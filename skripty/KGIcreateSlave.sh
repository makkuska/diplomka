...
psql -U postgres -p $mainPort -h 158.194.94.93 -c "SELECT pg_start_backup('x',true);"
scp -rv root@158.194.94.93:/etc/postgresql/$version/main $configdir
scp -rv root@158.194.94.93:/var/lib/postgresql/$version/main $datadir
psql -U postgres -p $mainPort -h 158.194.94.93 -c "SELECT pg_stop_backup();"
